function finalNets = shieldNN_design(rBar, lr, deltaFMax, vmax, sigma, radiusThresholds, freeControlThreshold)

    % Paremeters and Definitions

    % Bicycle parameters:
    %lr = 0.23; % total vehicle length = 2*lr = 0.46 
    ...
    %sigma = 0.25; % Scaling factor that determines minimum dist. to 
...
%     freeControlThreshold = 10;
    ...
    radiusThresholds = [radiusThresholds, freeControlThreshold];
    ...
    filePats = dir('finalNets*.onnx');
    fidx = -1;
    for ii = 1:length(filePats)
        [C,matches] = strsplit(filePats(ii).name,'\d*', 'DelimiterType','RegularExpression');
        if ~isempty(matches)
            fidx = max(fidx, str2num(matches{1}));
        end
    end
    fidx = fidx+1;
    ...
    for file_index = 1:length(finalNets)
        filename = strcat('finalNets',num2str(fidx),'_net',num2str(file_index) , '.onnx');
